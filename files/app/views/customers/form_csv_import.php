<?php
/**
 * OSPOS Customers CSV Import Form - Enhanced with Bulk Import
 * Replace: app/Views/customers/form_csv_import.php
 * 
 * Features:
...
<div class="csv-import-container">
    <h3>Import Customers from CSV</h3>
    ...
    <div id="single-tab" class="tab-content active">
        <div class="info-box">
            📋 Upload a single CSV file to import customers. The system will process each row and automatically sync to Mailchimp if configured.
        </div>
        ...
        <div class="download-template">
            <a href="<?= base_url('customers/getCsv') ?>" download>
    ...
    <div id="bulk-tab" class="tab-content">
        <div class="info-box">
            📦 Import multiple customer CSV files at once. Select files below to process them in sequence and automatically sync to your mailing list.
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('single-import-form').addEventListener('submit', async (e) => {
        ...
        try {
            const response = await fetch('<?= base_url("customers/postImportCsvFile") ?>', {
                method: 'POST',
                body: formData,
                headers: {
    ...
    document.getElementById('bulk-import-form').addEventListener('submit', async (e) => {
        ...
        try {
            const response = await fetch('<?= base_url("customers/postImportBulkCsvFiles") ?>', {
                method: 'POST',
                body: formData,
                headers: {
                    'X-Requested-With': 'XMLHttpRequest'
        ...
        let html = `
            <div style="padding: 15px; border: 1px solid #ddd; border-radius: 4px;">
                ...
                <div style="margin-top: 15px; font-size: 13px;">
                    ...
                    ${result.total_items_imported > 0 ? `<p>👥 <strong>Customers Imported:</strong> ${result.total_items_imported}</p>` : ''}
                </div>
        `;
